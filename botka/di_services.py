...
class UserService:
    ...
    async def get_or_create_user(self, user: User) -> TgUser:
        ...
        result = await self._session.execute(
            select(TgUser).where(TgUser.id == user.id)
        )
    ...
    async def add_mac(self, user_id: int, mac: str) -> bool:
        ...
        result = await self._session.execute(
            select(UserMac).where(
                and_(UserMac.tg_id == user_id, UserMac.mac == mac)
            )
        )
...
class CameraService:
    ...
    async def get_vortex_image(self) -> bytes | None:
        ...
        if not self._config.services.vortex_of_doom_cam:
            return None
        return await _get_camera_image(self._client, self._config.services.vortex_of_doom_cam)
